...
(function() {
    ...
    var parseEventData = (html) => {
        ...
        if(活动信息.match(/刷卡时间段：[-\d]+ [:\d]+.*?[-\d]+ [:\d]+/m)){
            ...
            //console.warn(`time doesn't match`, 活动信息)
                 ...
                 活动信息.match(/活动时长：(.*) 分钟.*?/m) ){
            let hdsc_match = 活动信息.match(/活动时长：([零一二三四五六七八九十0-9]*)个?(.*?) 分钟.*?/m);
            if(!hdsc_match) console.warn(活动信息)
    ...
    window.download_all_listof_activity_calendar_ical = async (e) => {
        ...
        await Promise.all(([
            ...
            let url=`/public/activity/activityList.action?pageNo=1&pageSize=9999999&categoryId=${target[0]}`;
